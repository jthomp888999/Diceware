...
# takes a word list and runs get_pair on it then
...
# joins into one number as of now not using the best prng
def randKey():
    key = [randint(1,6) for p in range(0,5)]
    keyInt = ''.join(map(str,key))
    return int(keyInt)

# main loop that should print the random word
# matched from the dictionary for as many words as requested
def main():
